...
const SliderList = () => {
    ...
    const handleDelete = (id: number, name: string) => {
        Swal.fire({
            icon: 'warning',
            title: 'Marka Sil',
            text: name + ' adlı  markayı silmek istediğinize emin misiniz?',
            ...
            if (result.value) {
                try {
                    ...
                    Swal.fire({
                        title: 'Hata!',
                        text: name + ' adlı marka silinemedi. Hata Alındı.',
                        icon: 'error',
                        confirmButtonText: 'Tamam',
                        customClass: { popup: 'sweet-alerts' }
                    });
    ...
    const handleChangeStatus = async (id: number) => { // ✅ currentStatus kaldırıldı
        try {
            ...
            setData((prevData: Brand[]) =>
                prevData.map((brand: Brand) =>
                    brand.id === id ? { ...brand, is_active: (brand.is_active ? 0 : 1) } : brand
                )
            );
            ...
            Swal.fire({
                icon: 'success',
                title: 'Başarılı!',
                text: `Marka durumu ${currentIsActiveText} olarak değiştirildi.`,
                confirmButtonText: 'Tamam'
            });
        } catch (error) {
            Swal.fire({
                icon: 'error',
                title: 'Hata!',
                text: 'Marka durumu değiştirilemedi. Lütfen tekrar deneyin.',
                confirmButtonText: 'Tamam'
            });
        }
    };
    ...
    useEffect(() => {
        dispatch(setPageTitle('Marka Listesi'));
    });
    useEffect(() => {
        const loadBrands = async () => {
            setLoading(true);
            try {
                const response = await BrandService.list(page, rowsPerPage, search, sortStatus);
        ...
        <div>
            <div className="panel mt-6">
                <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                    <h5 className="font-semibold text-lg dark:text-white-light">Marka Listesi</h5>
